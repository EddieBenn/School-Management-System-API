    ...
    let hotBolt = this.mwsExec.createBolt({
      ...
      onDone: async ({ req, res, results }) => {
        ...
        if (result.selfHandleResponse) {
          ...
          /**
           * qantra-pineapple returns an Array of error objects when validation fails.
           * e.g. [{ label, path, message, log, errors }]
           * Check for this first before the generic error/errors keys.
           */
